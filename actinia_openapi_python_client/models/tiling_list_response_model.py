...
class TilingListResponseModel(BaseModel):
    """
    Tiling process list reponse schema.
    """ # noqa: E501
    tiling_processes: List[TilingShortDescResponseModel] = Field(description="The list of all available tiling processes.")
    __properties: ClassVar[List[str]] = ["tiling_processes"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in tiling_processes (list)
        _items = []
        if self.tiling_processes:
            for _item in self.tiling_processes:
                if _item:
                    _items.append(_item.to_dict())
            _dict['tiling_processes'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            "tiling_processes": [TilingShortDescResponseModel.from_dict(_item) for _item in obj.get("tiling_processes")] if obj.get("tiling_processes") is not None else None
        })
        return _obj
