...
class OutputParameterExport(BaseModel):
    """
    The raster, vector or text file export parameter.
    """ # noqa: E501
    format: StrictStr = Field(description="The format of the output file in case of raster, strds, vector layer or text file export. Raster layer export supports only GTiff and COG format, STRDS layer export supports only GTiff format and all other formats are vector layer export formats. If the *PostgeSQL* format was chosen, a postgis database string *dbstring* must be provided  so that the GRASS GIS module *v.out.ogr knows to which PostgreSQL database it should be connect. The name of the output layer can be specified as *output_layer* for PostgreSQL database exports. Some GRASS GIS modules allow the export of text files. These files can be exported and provided as downloadable link as well.")
    type: StrictStr = Field(description="The type of the output. In case of the *file* option only text files are supported for export. In addition the *$file::unique_id* option must be used in the value parameter to export a file that was generated by a GRASS GIS module. Exported text and vector files will always be compressed with zip.")
    dbstring: Optional[StrictStr] = Field(default=None, description="The database string to be used to connect to a PostgreSQL database for vector export.")
    output_layer: Optional[StrictStr] = Field(default=None, description="Name for output PostgreSQL layer. If not specified, GRASS GIS vector map layer name is used.")
    __properties: ClassVar[List[str]] = ["format", "type", "dbstring", "output_layer"]

    @field_validator('format')
    def format_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('COG', 'GTiff', 'GPKG', 'SQLite', 'GML', 'GeoJSON', 'ESRI_Shapefile', 'CSV', 'TXT', 'PostgreSQL'):
            raise ValueError("must be one of enum values ('COG', 'GTiff', 'GPKG', 'SQLite', 'GML', 'GeoJSON', 'ESRI_Shapefile', 'CSV', 'TXT', 'PostgreSQL')")
        return value

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('raster', 'vector', 'file', 'strds'):
            raise ValueError("must be one of enum values ('raster', 'vector', 'file', 'strds')")
        return value
