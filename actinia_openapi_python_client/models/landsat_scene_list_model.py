...
class LandsatSceneListModel(BaseModel):
    """
    This schema defines the JSON input of the Landsat time series creator resource
    """ # noqa: E501
    atcor_method: StrictStr = Field(description="The method for atmospheric correction")
    strds: StrictStr = Field(description="The basename of the new space-time raster datasets (strds). One for each band will be created and the basename will be extended by a band specific suffix.")
    scene_ids: List[StrictStr] = Field(description="A list of Landsat scene names that should be downloaded and imported. Only scenes from a specific satelleite can be imported and represented as strds. Mixing of different satellites is not permitted. All bands will be imported and atmospherically corrected")
    __properties: ClassVar[List[str]] = ["atcor_method", "strds", "scene_ids"]

    @field_validator('atcor_method')
    def atcor_method_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('TOAR', 'DOS1', 'DOS4'):
            raise ValueError("must be one of enum values ('TOAR', 'DOS1', 'DOS4')")
        return value
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            "atcor_method": obj.get("atcor_method"),
            "strds": obj.get("strds"),
            "scene_ids": obj.get("scene_ids")
        })
        return _obj
