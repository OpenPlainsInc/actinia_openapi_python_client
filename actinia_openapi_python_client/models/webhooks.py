...
class Webhooks(BaseModel):
    ...
    """ # noqa: E501
    update: Optional[StrictStr] = Field(default=None, description="Specify a HTTP(S) GET/POST endpoint that should be called when a status update is available while the process chain is executed. The actinia JSON status response will be send as JSON content to the POST endpoint for each status update until the process finished. The GET endpoint, that must be available by the same URL as the POST endpoint, will be used to check if the webhook endpoint is available.")
    finished: StrictStr = Field(description="Specify a HTTP(S) GET/POST endpoint that should be called when the process chain was executed successful or unsuccessfully. The actinia JSON response will be send as JSON content to the POST endpoint after processing finished. The GET endpoint, that must be available by the same URL as the POST endpoint, will be used to check if the webhook endpoint is available.")
    __properties: ClassVar[List[str]] = ["update", "finished"]
