...
class OutputParameterMetadata(BaseModel):
    """
    The STAC file export parameter.
    """ # noqa: E501
    format: StrictStr = Field(description="Format of the metadata file. Only STAC is supported. The STAC item builder works just on raster export file.These files are accessible through a STAC Catalogand each export is stored as STAC Item")
    __properties: ClassVar[List[str]] = ["format"]

    @field_validator('format')
    def format_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('STAC'):
